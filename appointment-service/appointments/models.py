...
class TimeSlot(models.Model):
    ...
    def update_status(self):
        """Cập nhật trạng thái khả dụng dựa trên số lượng bệnh nhân hiện tại"""
        ...
        self.save(update_fields=['is_available', 'current_patients', 'status'])
    ...
    def add_patient(self):
        ...
        with transaction.atomic():
            ...
            if not time_slot.is_available:
                logger.warning(f"Trying to add patient to unavailable time slot {self.id}")
            ...
            time_slot.save(update_fields=['current_patients', 'is_available', 'status'])
            
            # Cập nhật object hiện tại để phản ánh thay đổi
            self.current_patients = time_slot.current_patients
            self.is_available = time_slot.is_available
    ...
    def save(self, *args, **kwargs):
        """Ghi đè phương thức save để đảm bảo tính nhất quán giữa is_available và current_patients"""
        # Đảm bảo current_patients không âm
        if self.current_patients < 0:
            self.current_patients = 0

        # Đảm bảo trạng thái khả dụng phản ánh đúng số lượng bệnh nhân
        if self.current_patients >= self.max_patients:
            self.is_available = False
            self.status = 'BOOKED'
        else:
